...
class AlertaAlerter(Alerter):
    ...
    def alert(self, matches):
        ...
        elastalert_logger.info("Alert sent to Alerta:{}".format(self.resource))
    ...
    def get_json_payload(self, match):
        ...
        graylog_link = self.rule.get('alerta_graylog_link','')
        if graylog_link <> '':
            link = resolve_string(graylog_link,match,self.missing_text)
            alerta_payload_dict['attributes']['graylog'] = "<a href=http://{} target=_blank>{}</a>".format(urllib.quote(link,safe='/:?&='),resolve_string(self.resource, match, self.missing_text))
...
#            self.body_labelname: self.create_alert_body(matches)})
        ...
        elastalert_logger.info("Alert sent to Alertmanager: {},{},{}".format(label,start_time.strftime("%Y-%m-%dT%H:%M:%S.%fZ"),end_time.strftime("%Y-%m-%dT%H:%M:%S.%fZ")))
